...
def get_evaluation_trends(evaluations):
    ...
    trends = (
        evaluations.filter(created_at__gte=start_date, status="approved")
        .annotate(month=TruncMonth("created_at"))
        .values("month")
        .annotate(count=Count("id"), avg_score=Avg("percentage_score"))
        .order_by("month")
    )
...
def calculate_compliance_rate(evaluations):
    ...
    on_time = evaluations.filter(
        status="approved", submitted_at__lte=F("end_date")
    ).count()
...
def generate_predictive_insights(evaluations):
    ...
    recent_avg = evaluations.filter(
        status="approved", created_at__gte=timezone.now() - timezone.timedelta(days=90)
    ...
    needs_calibration = evaluations.filter(
        status="approved", requires_calibration=True, calibration_completed=False
    ).count()
...
class EvaluationListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["pending_evaluations"] = evaluations.filter(
            status="in_progress"
        ).count()
        context["submitted_evaluations"] = evaluations.filter(
            status="submitted"
        ).count()
        context["completed_evaluations"] = evaluations.filter(status="approved").count()
        context["overdue_evaluations"] = evaluations.filter(
            end_date__lt=timezone.now().date(), status__in=["draft", "in_progress"]
        ).count()

        # Calculate percentages
        if total_count > 0:
            context["pending_percentage"] = round(
                (context["pending_evaluations"] / total_count) * 100, 1
            )
            context["submitted_percentage"] = round(
                (context["submitted_evaluations"] / total_count) * 100, 1
            )
            context["completed_percentage"] = round(
                (context["completed_evaluations"] / total_count) * 100, 1
            )
            context["overdue_percentage"] = round(
                (context["overdue_evaluations"] / total_count) * 100, 1
            )
        else:
...
class EvaluationDetailView(LoginRequiredMixin, DetailView):
        ...
        if user.role == "board_member":
            if obj.evaluator != user and obj.evaluatee != user:
                ...
                raise PermissionDenied(
                    "You can only view evaluations you're involved in."
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        answers = evaluation.answers.select_related("question").order_by(
            "question__order"
        )
        context["answers"] = answers

        # Get comments
        context["comments"] = evaluation.comments.select_related("author").order_by(
            "-created_at"
        )
        ...
        context["can_edit"] = user.role in [
            ...
            and evaluation.status in ["draft", "in_progress"]
        )
...
class EvaluationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if user.role not in ["company_secretary", "compliance_officer"]:
            if evaluation.evaluator != user or evaluation.status not in [
                "draft",
                "in_progress",
            ]:
                messages.error(
                    request, "You don't have permission to edit this evaluation."
                )
...
def take_evaluation(request, pk):
        ...
        with transaction.atomic():
            for question in questions:
                answer, created = EvaluationAnswer.objects.get_or_create(
                    evaluation=evaluation, question=question, defaults={}
                )
            ...
            if answered_questions.count() == required_questions.count():
                ...
                messages.info(
                    request, "Evaluation saved. Please complete all required questions."
                )
    ...
    return render(
        request,
        "evaluation/take_evaluation.html",
        {
...
def submit_evaluation(request, pk):
    ...
    answered_questions = evaluation.answers.filter(question__in=required_questions)

    if answered_questions.count() < required_questions.count():
        messages.error(
            request, "Please complete all required questions before submitting."
        )
...
def review_evaluation(request, pk):
    ...
    return render(
        request, "evaluation/review_evaluation.html", {"evaluation": evaluation}
    )
...
def add_question(request, template_pk):
    ...
    return render(
        request, "evaluation/question_form.html", {"form": form, "template": template}
    )
...
def evaluation_dashboard(request):
    ...
    if user.role == "board_member":
        evaluations = Evaluation.objects.filter(Q(evaluator=user) | Q(evaluatee=user))
    elif user.role in ["company_secretary", "compliance_officer"]:
    ...
    analytics = {
        ...
        "high_performers": evaluations.filter(
            status="approved", percentage_score__gte=90
        ).count(),
        "needs_improvement": evaluations.filter(
            status="approved", percentage_score__lt=70
        ).count(),
    }
    ...
    # My evaluations
    context["my_evaluations"] = evaluations.filter(evaluator=user).order_by(
        "-created_at"
    )[:5]
    context["evaluations_of_me"] = evaluations.filter(evaluatee=user).order_by(
        "-created_at"
    )[:5]
    ...
    context["pending_calibrations"] = evaluations.filter(
        status="calibration", requires_calibration=True
    ).count()
...
def populate_professional_templates(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(
                request,
                "Professional evaluation templates have been successfully loaded!",
            )
...
class CycleCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.created_by = self.request.user
        messages.success(
            self.request, f"Cycle '{form.instance.name}' created successfully."
        )
        return super().form_valid(form)
...
class CycleUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.role not in [
            ...
            messages.error(request, "You don't have permission to edit cycles.")
            return redirect("evaluation:cycle_list")
        return super().dispatch(request, *args, **kwargs)

    def form_valid(self, form):
        messages.success(
            self.request, f"Cycle '{form.instance.name}' updated successfully."
        )
        return super().form_valid(form)
