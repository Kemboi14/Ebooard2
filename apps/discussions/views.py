...
from .models import (
    ...
    PollVote, DiscussionSubscription, DiscussionTag, ThreadTag, PostReaction
)
from .forms import (
    DiscussionForumForm, DiscussionThreadForm, DiscussionPostForm, DiscussionPollForm,
    DiscussionSubscriptionForm, DiscussionTagForm, DiscussionSearchForm, PostEditForm
)
...
class ForumDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        threads = forum.threads.select_related('author').prefetch_related('tags').order_by('-is_pinned', '-last_activity')
...
class ThreadDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context['subscription_form'] = DiscussionSubscriptionForm(instance=subscription)
...
class ThreadCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Thread '{form.instance.title}' created successfully!")
...
class PostReactionView(LoginRequiredMixin, View):
    ...
    def post(self, request, post_pk):
        post = get_object_or_404(DiscussionPost, pk=post_pk)
...
def create_post(request, thread_pk):
    ...
    if thread.is_locked:
        ...
        return redirect('discussions:thread_detail', pk=thread_pk)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Set order
            last_order = thread.posts.filter(post_type='reply').order_by('-order').first()
    ...
    return render(request, 'discussions/create_post.html', {
        'form': form,
        'thread': thread
    })
...
def edit_post(request, post_pk):
    ...
    # Check permissions
    if post.author != request.user and request.user.role not in ['company_secretary', 'it_administrator']:
    ...
    return render(request, 'discussions/edit_post.html', {
        'form': form,
        'post': post,
        'thread': post.thread
    })
...
@login_required
def create_poll(request, thread_pk):
    ...
    return render(request, 'discussions/create_poll.html', {
        'form': form,
        'thread': thread
    })
...
def vote_poll(request, poll_pk):
    """Vote in a poll"""
    poll = get_object_or_404(DiscussionPoll, pk=poll_pk)
...
@login_required
def manage_subscription(request, thread_pk):
...
class TagDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        threads = DiscussionThread.objects.filter(
            thread_tags__tag=tag
        ).select_related('author', 'forum').order_by('-last_activity')
...
def search_discussions(request):
    ...
    if form.is_valid():
        ...
        threads = DiscussionThread.objects.select_related('author', 'forum').prefetch_related('tags')
    ...
    return render(request, 'discussions/search_results.html', {
        'form': form,
        'page_obj': page_obj,
        'threads': page_obj,
...
def discussions_dashboard(request):
    ...
    if user.role in ['company_secretary', 'it_administrator']:
        ...
        forums = DiscussionForum.objects.filter(
            access_level='public',
            is_active=True
        )
    ...
    context = {
        ...
        'recent_threads': DiscussionThread.objects.filter(
            forum__in=forums
...
class ForumCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Forum '{form.instance.name}' created successfully!")
